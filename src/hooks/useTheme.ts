...
import { ThemeDispatch } from "../interfaces";

export default function useTheme(ref: React.RefObject<any>): [boolean, ThemeDispatch] {
    const [theme, setTheme] = useState<boolean>(); // false = dark, true = light
    const dispatch: ThemeDispatch = () => {
    ...
    useEffect(() => {
        ...
        if (theme === undefined) { // searching for the user preference theme
            let storage: string | null = localStorage.getItem("theme"); // search for theme in localStorage
            let media: boolean = window.matchMedia("(prefers-color-scheme: light)").matches; // search for theme in media query
